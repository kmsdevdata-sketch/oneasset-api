...
| `role` | `UserRole` | Y | OneAsset 전역 권한 | 운영자 지정 또는 DB 복원 |
...
| 역할 | 의미 | 부여 기준 |
| --- | --- | --- |
| `USER` | 일반 대시보드 사용자 | Lazy Sync로 로컬 User 생성 시 기본값 |
| `ADMIN` | 관리자 페이지 접근 권한을 가진 사용자 | MVP1에서는 운영자가 DB 또는 별도 운영 경로로 지정 |

`UserRole`은 OneAsset 전체 관리자 권한을 표현한다. 프로젝트 내부의 owner/member 권한은 `ProjectMember.role`로 별도 관리한다.
...
| `users` | `role` | `role` | NOT NULL | enum string |
