...
## 2026-07-12
- 발급된 raw API Key를 Developer API요청에서 받아서 DB의 해시와 미칭 -> 어떤 프로젝트 요청인지 식별 
  - 인증결과 객체 : 우선 작업하면서 정확히 어떤 필드가 필요할지 식별후 결정하기 
  - API Key 인증 유스케이스 : 사용자 헤더에서 `rawKey`받아와야 하니까 매개변수로 사용 반환값은 인증결과 객체 사용 
    - rawKey -> 해시변환 -> 해당 해시로 DB조회 -> 해당 컬럼의 projectId확인후 반환 
    - public Optional<ApiKey> findActiveByHash(ApiKeyHash hash) 는 기존에 테스트에서만 사용되어서 포트 메서드는 아니였지만 포트로 전환
    - 요청유저와 API키의 프로젝트 아이디 동일한지 검사를 하려했으나 사용자 어플리케이션에서 요청한걸 염두해서(Auth~principal사용불가) 우선은 API키 신뢰하기로 결정
  - API인증 추출기 만들기 
    - 사용자 어플리케이션에서 오는 요청은 api키 검증이 계속하여 일어나니 별도 컨트롤러로 분리하기보다 우선 추출클래스를 별도로 만들기로함 
    - AuthenticatedApiKey - 유즈케이스 추출한 결과를 뱉어내는걸로 결정 
    - ExtractApiKeyCommand - 사용자 요청헤더에서 raw키만 분리하여 뱉어내는걸로 결정 
      - 차라리 어노테이션으로 만들어버리는게 나을거같기도함 : 아니면 필터도? 우선 클래스로 만들고 추후 사용자 API만들때 검증주기 확인하여 변경고려
    - 근데 생각해보니까 @RequestHeader 로 바로 받아와서 유즈케이스에 넘기면 되는데 Extractor가 굳이 필요한가 싶어서 그냥지움 
